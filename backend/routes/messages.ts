...
export async function markMessageReadByCustomer(
  ...
  const participantList =
    await getParticipantListOfConversation(conversationId);
...
export async function markMessageReadByAgent(
  conversationId: bigint,
  messageId: bigint,
) {
  const updatedMessage = await prisma.message.update({
...
export async function messageRoutes(app: FastifyInstance) {
  app.get("/conversations/:conversationId/messages", async (request, reply) => {
    try {
      ...
      if (!conversation) {
        ...
        return reply
          .status(404)
          .send({ error: "Conversation with given id not found" });
      }
  ...
  app.post<{
    ...
    try {
      ...
      const participant = await findParticipantInConversation(
        BigInt(conversationId),
        BigInt(senderId),
      );
      if (!participant) {
        return reply
          .status(403)
          .send({ error: "User is not a participant in this conversation" });
      }
      ...
      if (!body && !fileUrl) {
        return reply
          .status(400)
          .send({ error: "Message must have a body or a fileUrl" });
      }
      ...
      const participantUserIds = participantList.map((participant) =>
      ...
      const eventType =
        currentRole == "CUSTOMER"
          ? "NEW_MESSAGE_BY_CUSTOMER"
          : "NEW_MESSAGE_BY_AGENT";
  ...
  app.patch<{
    ...
    "/conversations/:conversationId/messages/:messageId/read/customer",
    async (request, reply) => {
      const { conversationId, messageId } = request.params;
      try {
        const serialisedMessage = await markMessageReadByCustomer(
          BigInt(conversationId),
          BigInt(messageId),
        );
        ...
        reply
          .status(500)
          .send({ error: "Failed to mark message as read by customer" });
      }
    },
  );
  ...
  app.patch<{
    ...
    "/conversations/:conversationId/messages/:messageId/read/agent",
    async (request, reply) => {
      const { conversationId, messageId } = request.params;
      try {
        const serialisedMessage = await markMessageReadByAgent(
          BigInt(conversationId),
          BigInt(messageId),
        );
        reply.send({ success: true, message: serialisedMessage });
        ...
        reply
          .status(500)
          .send({ error: "Failed to mark message as read by agent" });
      }
    },
  );
