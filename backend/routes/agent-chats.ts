...
export async function agentChatRoutes(fastify: FastifyInstance) {
  fastify.get("/getAgentChats", async (request, reply) => {
    ...
    const userAsParticipantList = await getAgentChats(userId);
  ...
  fastify.delete("/deleteChatForAll", async (request, reply) => {
    try {
      ...
      if (!conversationId) {
        return reply
          .status(400)
          .send({ success: false, message: "conversationId is required" });
      }

      await removeAllParticipantsFromConversation(
      ...
      return reply
        .status(500)
        .send({ success: false, message: error.message || "Unknown error" });
    }
  });
}
